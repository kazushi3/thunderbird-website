...
def build_ical(provider, locale, years_to_generate):
    ...
    for i in range(0, years_to_generate):
        ...
        for calendar_type in [CalendarTypes.NATIONAL, CalendarTypes.OBSERVANCE]:
            try:
                holidays = provider.build(locale, year, {'calendar_type': calendar_type.value})
...
def build_calendars(provider: Provider, locales):
    """
    ...
    for locale, country_name in locales.items():
        ...
        ical = build_ical(provider, locale, years_to_generate)
